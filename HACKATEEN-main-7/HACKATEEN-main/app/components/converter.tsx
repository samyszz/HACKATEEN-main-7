  ...
  async function handleConvert() {
    ...
    try {
      ...
      if (data.success === false) {
        console.error(t("Converter.api_error", { info: data.error.info }));
        alert(t("Converter.api_error", { info: data.error.info }));
        setResult(null);
      } else if (typeof data.result === "number") {
        setResult(data.result);
      } else {
        console.error(t("Converter.unexpected_api_response"));
        alert(t("Converter.unexpected_api_response"));
        setResult(null);
      }
    } catch (error) {
      console.error(t("Converter.api_connection_error"));
    ...
    <>
      ...
      >
        ...
        <div className="absolute inset-0 md:hidden z-0">
          <Image
            src="/Money.png"
            alt={t("Converter.money_image_alt")}
            layout="fill"
            objectFit="cover"
